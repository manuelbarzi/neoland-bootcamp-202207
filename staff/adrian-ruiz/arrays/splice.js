function splice(array, start, deleteCount, ...elements){
    ...
    if(start > 0 && deleteCount === 0 && elements.length > 0){
        let x = start   // Para controlar los elementos que quiero reemplazar
        
        // 1 - Guardo el ultimo index del array original
        // 2 - Aumento la longitud del array segun la cantidad de elementos que quiero introducir
        // 3 - Empiezo el bucle desde el final del array (Ya ampliado) hasta la posición de inicio + start (Ya que los X huecos donde van los elementos me dan igual)
        // 4 - Muevo los elementos restantes hasta el final del array de derecha a izquierda
        // 5 - Reduzco el indice 

        let index = array.length-1 // 1
        array.length += elements.length // 2
        for(let i = array.length-1; i >= start + elements.length; i--){ //3
            array[i] = array[index] //4
            index--
        }
